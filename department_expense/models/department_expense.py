...
class ReportDepartment(models.AbstractModel):
    _name = 'report.department_expense.report_department'
    ...
    def render_html(self, departmentids, data=None):
        ...
        self._cr.execute("""with dep_total AS (SELECT * from (SELECT pc.name, ail.price_subtotal,ai.date_invoice FROM account_invoice_line ail LEFT JOIN 
                        ...
                        ai.type = 'in_invoice' UNION ALL SELECT pc.name department_name, - ail.price_subtotal,ai.date_invoice FROM account_invoice_line ail 
                        ...
                        ai.state IN ('open','paid') AND ai.type = 'in_refund')b where date_invoice>=%s and date_invoice<=%s) 
        ...
        return self.env['report'].render('department_expense.report_department', departmentargs)
