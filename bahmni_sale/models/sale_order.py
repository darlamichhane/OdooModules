...
class SaleOrder(models.Model):
    ...
    @api.depends('order_line.price_total', 'discount', 'chargeable_amount','my_discount_amnt','my_discount_type')
    def _amount_all(self):
        ...
        for order in self:
            ...
            if order.my_discount_type == 'categ_amnt':
                amount_total = amount_untaxed + amount_tax 
            else:
                amount_total = amount_untaxed + amount_tax - order.my_discount_amnt
...
################sagar edit
    @api.one
    @api.depends('order_line.discount_line_amount','my_discount_percentage','my_fxd_discount_amnt','my_discount_type')
    def _compute_my_disc_amount(self):
        ...
        if self.my_discount_type == 'categ_amnt':
                self.my_discount_amnt =  sum(line.discount_line_amount for line in self.order_line)
    ...
    @api.onchange('partner_id','my_discount_type')
    def get_auto_cate(self):
        if self.partner_id and self.my_discount_type == 'categ_amnt':
            for order in self:
                # if order.my_discount_type == 'categ_amnt':
                    disc_obj = self.env['discount.setup'].search([('code','ilike',order.partner_id.ref)],limit=1)    
                    order.mydiscount_id = disc_obj.id
...
#################
    ...
    # sagar fields
    mydiscount_id = fields.Many2one('discount.setup',string='Discount Category')
    my_discount_type = fields.Selection([('none_amnt', 'No Discount'),
                                      ('fixed_amnt', 'Fixed'),
                                      ('percentage_amnt', 'Percentage'),('categ_amnt', 'Category')],string="Discount Method",default='none_amnt')
    my_discount_amnt = fields.Monetary(string="Discount",compute='_compute_my_disc_amount',store=True)
    # discount_cate_amount = fields.Monetary(string="Discount Amount",compute='_compute_my_disc_amount',store=True)
    my_discount_percentage = fields.Float(string="Discount Percentage")
    my_fxd_discount_amnt = fields.Monetary(string="Discount Amount")
    # my_disc_acc_id = fields.Many2one('account.account',string="Discount Account Head")
    ...
    def _prepare_invoice(self):
        ...
        invoice_vals = {
            ...
            'mydiscount_id': self.mydiscount_id.id,
            'my_discount_type': self.my_discount_type,
            'my_discount_amnt':self.my_discount_amnt,
            'my_discount_percentage':self.my_discount_percentage,
            'my_fxd_discount_amnt':self.my_fxd_discount_amnt,
            # 'my_disc_acc_id':self.my_disc_acc_id.id
    ...
    def action_confirm(self):
        ...
        for lines in self.order_line:
            if not lines.doctor_id:
                raise UserError(_('Please enter doctor id in order lines.'))
